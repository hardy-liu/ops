#!/bin/bash
# Data: 2016-01-07
#nginx相关的软件包需要放到/root/nginx目录下，脚本可以在任何位置运行
#安装依赖包
...
./configure --error-log-path=/var/log/nginx/error.log --http-log-path=/var/log/nginx/access.log --pid-path=/var/run/nginx.pid --lock-path=/var/lock/nginx.lock --user=nginx --group=nginx --with-http_ssl_module --with-http_flv_module --with-http_stub_status_module --with-http_gzip_static_module --with-file-aio --with-pcre --with-http_v2_module --with-http_realip_module --with-http_gunzip_module --with-http_sub_module --with-ld-opt="-Wl,-rpath,/usr/local/lib" --add-module=../lua-nginx-module-0.9.19 --add-module=../ngx_devel_kit-0.2.19 --add-module=../redis2-nginx-module-0.12 --add-module=../echo-nginx-module-0.58 --add-module=../set-misc-nginx-module-0.29 --add-module=../nginx-module-vts-0.1.8 --add-module=../ngx_cache_purge-2.3 || exit 1
make || exit 1
make install || exit 1
