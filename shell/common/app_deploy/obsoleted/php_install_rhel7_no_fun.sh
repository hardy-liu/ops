...
if [[ $? -eq 0 ]]; then
	...
	for i in ${phpConfDir}/php-fpm.conf; do
		sed -i 's/pm.max_children = 5/pm.max_children = 50/g' $i
		sed -i 's/pm.start_servers = 2/pm.start_servers = 5/g' $i
		sed -i 's/pm.min_spare_servers = 1/pm.min_spare_servers = 5/g' $i
		sed -i 's/pm.max_spare_servers = 3/pm.max_spare_servers = 35/g' $i
...
tar zxf $phpRedisPackage || exit 1
cd ${phpRedisVersion}
phpize
